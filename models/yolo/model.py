...
class YOLO(Model):
    ...
    def __init__(self, model="yolo11n.pt", task=None, verbose=False):
        ...
        if isinstance(model, str) and "-world" in model and os.path.splitext(model)[1] in {".pt", ".yaml", ".yml"}:
            path = Path(model)
...
class YOLOWorld(Model):
    ...
    def __init__(self, model="yolov8s-world.pt", verbose=False) -> None:
        ...
        if not hasattr(self.model, "names"):
            self.model.names = yaml_load(ROOT / "models/datasets/coco8.yaml").get("names")
