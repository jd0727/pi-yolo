...
class DetMetrics(SimpleClass):
    ...
    def process(self, tp, conf, pred_cls, target_cls):
        ...
        results_sub = results[2:]
        #######################################################################################################
        tp, fp, p, r, f1, ap_all, class_index, p_curve, r_curve, f1_curve, x, prec_values = results
        dframe = pd.DataFrame(columns=['Class', 'TP', 'FP', 'Prec', 'Recl', 'F1', 'AP', 'AP50', 'AP75', ])
        for _i, c in enumerate(class_index):
            dframe = pd.concat([dframe, pd.DataFrame(
                {
                    'Class': self.names[c] if c in self.names else str(c),
                    'AP': np.mean(ap_all[_i]),
                    'AP50': ap_all[_i, 0],
                    'AP75': ap_all[_i, 5],
                    'Prec': p[_i],
                    'Recl': r[_i],
        ...
        dframe = pd.concat([dframe, pd.DataFrame(
            {
                'Class': 'Total',
                'AP': np.mean(dframe['AP']),
                'AP50': np.mean(dframe['AP50']),
                'AP75': np.mean(dframe['AP75']),
                'Prec': np.mean(dframe['Prec']),
                'Recl': np.mean(dframe['Recl']),
                'F1': np.mean(dframe['F1']),
                'TP': np.sum(dframe['TP']),
                'FP': np.sum(dframe['FP'])
            }, index=[0])])
        dframe.to_excel(os.path.join(self.save_dir, 'result.xlsx'), index=False)
        #######################################################################################################

        self.box.nc = len(self.names)
        self.box.update(results_sub)
        return dframe
