...
from cfg import MODELS, TASK2DATA, TASKS
...
def test_track_stream():
    ...
    for gmc in "orb", "sift", "ecc":
        with open(ROOT / "models/trackers/botsort.yaml", encoding="utf-8") as f:
...
def test_all_model_yamls():
    """Test YOLO model creation for all available YAML configurations in the `models/models` directory."""
    for m in (ROOT / "models" / "models").rglob("*.yaml"):
        if "rtdetr" in m.name:
...
def test_data_utils():
    ...
    from data.utils import HUBDatasetStats, autosplit
    from utils.downloads import zip_directory

    # from utils.files import WorkingDirectory
...
def test_data_converter():
    ...
    from data.converter import coco80_to_coco91_class, convert_coco
...
def test_cfg_init():
    """Test configuration initialization utilities from the 'ultralytics.models' module."""
    from cfg import check_dict_alignment, copy_default_cfg, smart_value
...
def test_utils_init():
    ...
    from utils import get_git_branch, get_git_origin_url, get_ubuntu_version, is_github_action_running
...
def test_utils_torchutils():
    ...
    from utils.torch_utils import get_flops_with_torch_profiler, profile, time_sync
...
def test_utils_downloads():
    """Test file download utilities from utils.downloads."""
...
def test_utils_files():
    ...
    from utils.files import file_age, file_date, get_latest_run, spaces_in_path
...
def test_utils_patches_torch_save():
    ...
    from utils.patches import torch_save
...
def test_nn_modules_conv():
    ...
    from nn.modules.conv import CBAM, Conv2, ConvTranspose, DWConvTranspose2d, Focus
...
def test_nn_modules_block():
    ...
    from nn.modules.block import C1, C3TR, BottleneckCSP, C3Ghost, C3x
...
def test_hub():
    ...
    from hub import export_fmts_hub, logout
    from hub.utils import smart_request
...
def test_classify_transforms_train(image, auto_augment, erasing, force_color_jitter):
    ...
    from data.augment import classify_augmentations
...
def test_yolo_world():
    ...
    from models.yolo.world.train_world import WorldTrainerFromScratch
