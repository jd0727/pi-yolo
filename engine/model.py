...
from nn.tasks import attempt_load_one_weight, guess_model_task, nn, yaml_model_load, DetectionModelNN, DetectionModel
from utils import (
    ARGV,
    ASSETS,
    DEFAULT_CFG_DICT,
...
class Model(nn.Module):
    ...
    def __init__(
            self,
            model: Union[str, Path, nn.Module] = "yolo11n.pt",
            task: str = None,
            verbose: bool = False,
    ) -> None:
        """
        ...
        if isinstance(model, str):
            ...
            self._new_nn(model=model, task=task, verbose=verbose)
    ...
    def _new_nn(self, task=None, model=None, verbose=False) -> None:
        self.cfg = {}
        self.task = 'detect'
        self.model = model
    ...
    def train(
        ...
        overrides = yaml_load(checks.check_yaml(kwargs["models"])) if kwargs.get("models") else self.overrides
        custom = {
            # NOTE: handle the case when 'models' includes 'data'.
        ...
        if not args.get("resume"):  # manually set model only if not resuming
            if isinstance(self.model, DetectionModel):
                ...
                self.trainer.model = DetectionModelNN(self.model, )
                self.model = self.trainer.model
                # print('RANK',RANK)
                # if RANK>=0:
                #     self.model.to(torch.device('cuda:%d' % RANK))

        self.trainer.hub_session = self.session  # attach optional HUB session
        self.trainer.train()
        # Update model and models after training
        if RANK in {-1, 0}:
