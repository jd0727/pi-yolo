    ...
    Arguments received: {str(['yolo'] + ARGV[1:])}. Ultralytics 'yolo' commands use the following syntax:
        ...
        Where   TASK (optional) is one of {TASKS}
                ...
                ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                    See all ARGS at https://docs.ultralytics.com/usage/cfg or with 'yolo models'
...
def cfg2dict(cfg):
    ...
    Notes:
        - If models is a path or string, it's loaded as YAML and converted to a dictionary.
        - If models is a SimpleNamespace object, it's converted to a dictionary using vars().
        - If models is already a dictionary, it's returned unchanged.
    """
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None):
    ...
    Examples:
        >>> from models import get_cfg
        >>> config = get_cfg()  # Load default configuration
        >>> config = get_cfg("path/to/config.yaml", overrides={"epochs": 50, "batch_size": 16})

    Notes:
        - If both `models` and `overrides` are provided, the values in `overrides` will take precedence.
    ...
    if overrides:
        ...
        cfg = {**cfg, **overrides}  # merge models and overrides dicts (prefer overrides)
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    from utils.files import increment_path  # for output directory path update
...
def entrypoint(debug=""):
    ...
    special = {
        ...
        "models": lambda: yaml_print(DEFAULT_CFG_PATH),
        "hub": lambda: handle_yolo_hub(args[1:]),
        "login": lambda: handle_yolo_hub(args),
        "logout": lambda: handle_yolo_hub(args),
        "copy-models": copy_default_cfg,
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        ...
        if "=" in a:
            try:
                ...
                if k == "models" and v is not None:  # custom.yaml passed
                    LOGGER.info(f"Overriding {DEFAULT_CFG_PATH} with {v}")
                    overrides = {k: val for k, val in yaml_load(checks.check_yaml(v)).items() if k != "models"}
                else:
                    overrides[k] = v
            except (NameError, SyntaxError, ValueError, AssertionError) as e:
...
def copy_default_cfg():
    ...
    Examples:
        ...
        # Example YOLO command with this new custom models:
        #   yolo models='/path/to/current/directory/default_copy.yaml' imgsz=320 batch=8
    ...
    LOGGER.info(
        ...
        f"Example YOLO command with this new custom models:\n    yolo models='{new_file}' imgsz=320 batch=8"
    )
