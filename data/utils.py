...
def verify_image_label(args, cls_names_inv=None):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            ##############################################
            root = ET.parse(lb_file).getroot()
            size = root.find('size')
            w = int(size.find('width').text)
            h = int(size.find('height').text)
            assert shape==(w,h),'Err label size'
            lb = []
            for obj in root.iter('object'):
                cls_name = obj.find('name').text
                if cls_name not in cls_names_inv.keys():
                    print('Err name', cls_name)
                cls_id = cls_names_inv.get(cls_name, 0)
                xmlbox = obj.find('bndbox')
                b = (float(xmlbox.find('xmin').text) / w,
                     float(xmlbox.find('ymin').text) / h,
                     float(xmlbox.find('xmax').text) / w,
                     float(xmlbox.find('ymax').text) / h)
                b = xyxy2xywh(np.array(b))
                lb.append(np.concatenate([[float(cls_id)], b], axis=0))
            lb = np.array(lb)
            ##############################################
